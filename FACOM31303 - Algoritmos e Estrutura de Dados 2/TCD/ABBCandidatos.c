...
int remove_ArvBin(ArvBin *raiz, Candidato *valor) { //AQUIIIIIIIII
    if(raiz == NULL) 
        return 0;
    struct NO* ant = NULL;
    struct NO* atual = *raiz;
...
Candidato *busca_ArvBin(ArvBin *raiz, char *cep){ //AQUIIIIIII
...
ArvBin *lerArquivoEnderecos(char *enderecoArquivo, int op) { //AQUIIIIIII
...
void destroiCandidato(Candidato *cand){
    if(cand == NULL) return;
    free(cand);
}